...
export async function GET(
  ...
  try {
    ...
    const responseData = bids.map((bid) => {
      const authorName =
        Array.isArray(bid.author) && bid.author.length > 0
          ? bid.author[0]?.name
          : "Usuário Anônimo";
...
export async function POST(
  ...
  try {
    ...
    if (profileError || !userProfile) {
      throw new Error(
        "Perfil de usuário não encontrado para verificação de crédito."
      );
    }

    if (userProfile.credit_balance < amount) {
      return NextResponse.json(
        { error: "Créditos insuficientes para este lance." },
        { status: 400 }
      );
    }
    ...
    if (amount <= minimumRequiredBid) {
      return NextResponse.json(
        {
          error: `O lance deve ser maior que ${minimumRequiredBid.toLocaleString(
            "pt-BR",
